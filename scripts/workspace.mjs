 ...
 * The mixed pre-split repository remains the first default; after splitting, the sibling zeitberg-data checkout is discovered automatically.
 ...
 */
export async function resolveWorkspaceRoot(requestedRoot, configPath = "zeitberg.json") {
    ...
    const candidates = [CODE_ROOT, resolve(CODE_ROOT, "..", "zeitberg-data")];
    for (const candidate of candidates) {
        if (await pathExists(resolveWorkspaceFile(candidate, normalizedConfigPath))) return candidate;
    }
    throw new Error(
        `Could not find ${normalizedConfigPath}; pass --workspace PATH or clone zeitberg-data next to the code repository.`,
    );
}
 ...
 * @param {string} repositoryPath Path declared by zeitberg.json.
 * @returns {string}
 */
 ...
 */
export async function loadWorkspace(requestedRoot, configPath = "zeitberg.json") {
    const normalizedConfigPath = normalizeRepositoryPath(configPath, "workspace config path");
